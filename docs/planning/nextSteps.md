...
[ ] **Task 10 — Freeze, calibrate, package, and only then attempt optional improvements** *(Task 10A inference skeleton implemented and merged to `main`; Task 10B model selected, real predictor wired in, calibration deliberately skipped (see below), resource profiling not yet run, `final_test` not started)*
  ...
  - [ ] Run the sealed `final_test` once after weights, calibration, threshold, and feature flags are frozen; generate robustness, ablation, generalization, and error-analysis artifacts. *(Blocked: requires separate explicit approval before running, regardless of readiness.)*
