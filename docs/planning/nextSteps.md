...
[ ] **Task 10 — Freeze, calibrate, package, and only then attempt optional improvements** *(Task 10A inference skeleton implemented and merged to `main`; Task 10B model selected, real predictor wired in, calibration deliberately skipped (see below), resource profiling not yet run; `final_test` evaluation built and tested but not yet run — blocked on Colab, see below)*
  ...
  - [x] Build the sealed `final_test` evaluation, to run once after weights, calibration, threshold, and feature flags are frozen. *(`src/cya_detector/evaluation/final_test.py`'s `evaluate_final_test` is the only code in the repository permitted to read `final_test` rows: requires an explicit `confirm_final_test_read=True`, validates every row is a direct fixed-Q96 matched-clean view with a hash-verified image before scoring it, and refuses outright — no resume, no overwrite — if a prior result already exists at the output root. Wired to `scripts/run_final_test.py` (additionally gated behind `--i-understand-this-is-irreversible`) and `notebooks/11_final_test.ipynb`. 8 tests pass against synthetic fixtures. **Not yet run**: the real `final_test` images live in a 245MB Drive archive (`task2_stagea_bundle.tar.gz`) impractical to pull through this session's chat-based Drive connector — this must be run on Colab, where Drive/GPU access already works, using notebook 11 or the equivalent `make` invocation once one exists. This is the one remaining step that actually touches the sealed set; do not run it until the model/calibration/threshold above are genuinely final.)*
  ...
  - [ ] Complete when a fresh environment can reproduce training/evaluation artifacts and run the local JSON inference contract without external APIs. *(Remaining gap: resource profiling on target hardware, then `final_test`.)*
